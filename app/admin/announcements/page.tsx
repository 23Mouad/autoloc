...
import { Plus, Pencil, Trash2, Megaphone, Upload, X, Loader2, Globe } from "lucide-react";
...
export default function AdminAnnouncementsPage() {
  ...
  const [form, setForm] = useState({ title: "", content: "", image: "", isGlobal: true });
  ...
  const resetForm = () => {
    setForm({ title: "", content: "", image: "", isGlobal: true });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  const handleEdit = (a: AnnouncementItem) => {
    setForm({ title: a.title, content: a.content, image: a.image || "", isGlobal: a.isGlobal });
    setEditingId(a.id);
    setShowForm(true);
  };
  ...
  const getOwnerName = (ownerId: AnnouncementItem["ownerId"]) => {
    if (typeof ownerId === "object" && ownerId?.name) return `${ownerId.name} ${ownerId.lastName || ""}`.trim();
    return "Admin";
  };
        ...
        <div className="mb-6 bg-bg-secondary border border-border rounded-xl p-6 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-text-secondary">Global announcement (visible to everyone)</span>
            </label>
            <button
              type="submit"
              disabled={saving}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {a.isGlobal && (
                      <span className="flex items-center gap-1 px-2 py-0.5 bg-accent/10 text-accent text-[10px] font-bold uppercase rounded-full">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-text-muted">
                    <span>By {getOwnerName(a.ownerId)}</span>
                    <span>·</span>
